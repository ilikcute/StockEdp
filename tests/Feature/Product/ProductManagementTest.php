...
class ProductManagementTest extends TestCase
{
    ...
    public function test_filter_products_by_unit(): void
    {
        $unit2 = Unit::create(['code' => 'BOX', 'name' => 'Box', 'symbol' => 'box']);
        ...
        $response = $this->actingAs($this->viewerUser)
            ->getJson("/api/v1/products?unit_id={$unit2->id}");

        $response->assertStatus(200);
        $this->assertCount(1, $response->json('data'));
    }

    public function test_filter_products_by_category_and_unit_together(): void
    {
        $cat2 = Category::create(['code' => 'CAT-02', 'name' => 'Furniture']);
        $unit2 = Unit::create(['code' => 'BOX', 'name' => 'Box', 'symbol' => 'box']);
        ...
        $response = $this->actingAs($this->viewerUser)
            ->getJson("/api/v1/products?category_id={$cat2->id}&unit_id={$unit2->id}");

        $response->assertStatus(200);
        $this->assertCount(1, $response->json('data'));
    }

    public function test_filter_products_with_empty_parameters_returns_all(): void
    {
        Product::factory()->create([
            'category_id' => $this->category->id,
            'unit_id' => $this->unit->id,
        ...
        $response = $this->actingAs($this->viewerUser)
            ->getJson('/api/v1/products?category_id=&unit_id=&search=&is_active=');
    ...
    public function test_admin_can_create_product_with_unit_price(): void
    {
        $response = $this->actingAs($this->admin)
            ->postJson('/api/v1/products', [
                'sku' => 'SKU-PRICE-1',
                'name' => 'Produk Berharga',
                'category_id' => $this->category->id,
                'unit_id' => $this->unit->id,
                'minimum_stock' => 10,
                'unit_price' => 75000.50,
        ...
        $response->assertStatus(201)
            ->assertJsonPath('data.unit_price', '75000.50');
    ...
    public function test_admin_can_update_product_unit_price(): void
    {
        $product = Product::factory()->create([
            'category_id' => $this->category->id,
            'unit_id' => $this->unit->id,
        ...
        $response->assertStatus(200)
            ->assertJsonPath('data.unit_price', '99000.00');
