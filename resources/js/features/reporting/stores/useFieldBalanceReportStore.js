...
export const useFieldBalanceReportStore = defineStore('fieldBalanceReport', {
    state: () => ({
        data: [],
        meta: null,
        summary: null,
    ...
    actions: {
        async fetchReport(filters = {}) {
            ...
            try {
                const response = await reportingApi.getFieldBalances(filters);
                ...
                this.error = normalized.status === 422
                    ? null
                    : (normalized.message || 'Gagal memuat laporan saldo teknisi');
            } finally {
