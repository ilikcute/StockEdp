  ...
  (newItems) => {
    newItems.forEach((item) => {
      if (countInputs[item.id] === undefined) {
        const q = item.counted_quantity;
        countInputs[item.id] =
          q !== null && q !== undefined
            ? (String(q).endsWith('.0000') ? String(q).slice(0, -5) : String(q).replace(/0+$/, '').replace(/\.$/, ''))
            : '';
      }
    });
  },
...
const handleCountBlur = (itemId) => {
  if (countInputs[itemId]) {
    ...
    if (norm !== null) {
      countInputs[itemId] = norm.endsWith('.0000') ? norm.slice(0, -5) : norm.replace(/0+$/, '').replace(/\.$/, '');
    }
  }
};
...
const handleUnexpectedBlur = () => {
  if (unexpectedForm.counted_quantity) {
    ...
    if (norm !== null) {
      unexpectedForm.counted_quantity = norm.endsWith('.0000') ? norm.slice(0, -5) : norm.replace(/0+$/, '').replace(/\.$/, '');
    }
  }
};
