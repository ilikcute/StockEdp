...
import { formatRupiah } from '@/shared/utils/formatters.js';
...
export function useInventoryBarcodeScanner(onProductFound, onScanComplete) {
    ...
    const processQueue = async () => {
        ...
        statusMessage.value = `Mencari barcode / SKU ${barcode}...`;
        ...
        try {
            ...
            status.value = 'FOUND';
            const priceStr = product.unit_price !== undefined && product.unit_price !== null ? ` [${formatRupiah(product.unit_price)}]` : '';
            statusMessage.value = `✓ ${product.name} (${product.sku})${priceStr} ditemukan.`;
            ...
            if (errCode === 'BARCODE_NOT_FOUND' || err.response?.status === 404) {
                ...
                statusMessage.value = `Barcode / SKU "${barcode}" tidak ditemukan.`;
            } else if (errCode === 'PRODUCT_INACTIVE' || err.response?.status === 409) {
                status.value = 'INACTIVE';
                statusMessage.value = `Produk dengan barcode / SKU "${barcode}" sudah tidak aktif.`;
            } else {
                status.value = 'ERROR';
                statusMessage.value = errMsg || 'Gagal memproses pencarian barcode / SKU.';
            }
        } finally {
            isProcessing.value = false;
            queueLength.value = scanQueue.length;
