...
class CreateStockAdjustmentRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        if ($this->user()?->hasRole(RoleCode::ADMIN->value)) {
            return true;
        }

        $allowedLocations = $this->user() ? $this->user()->getAllowedLocationIds() : [];
    ...
    {
        throw new AuthorizationException('Anda tidak memiliki hak akses untuk lokasi gudang yang dipilih.');
    }
