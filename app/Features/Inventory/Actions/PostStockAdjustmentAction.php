...
class PostStockAdjustmentAction
{
    ...
    public function execute(StockAdjustment $adjustment, int $userId): StockAdjustment
    {
        return DB::transaction(function () use ($adjustment, $userId) {
            ...
            if ($user && ! $user->hasRole(RoleCode::ADMIN) && ! in_array($lockedAdjustment->location_id, $user->getAllowedLocationIds(), true)) {
            ...
            foreach ($lockedAdjustment->items as $item) {
                $dtos[] = new StockChangeDTO(
                    ...
                    userId: $userId,
                    occurredAt: $lockedAdjustment->adjustment_date ? $lockedAdjustment->adjustment_date->format('Y-m-d H:i:s') : now()->format('Y-m-d H:i:s')
                );
            }
            ...
            return $lockedAdjustment->load(['items.product', 'location', 'creator', 'updater', 'poster', 'canceler']);
        });
    }
}
