...
class StockAdjustmentRepository implements StockAdjustmentRepositoryInterface
{
    public function getPaginatedAdjustments(
        ...
        // Scope by allowed locations. If Admin, do not scope. If empty, return empty paginator scope
        if (auth()->user()?->hasRole(RoleCode::ADMIN->value)) {
            // Admin has global access to all locations
