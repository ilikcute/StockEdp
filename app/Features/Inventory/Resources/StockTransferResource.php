...
class StockTransferResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'transfer_type' => $this->transfer_type?->value ?? 'TRANSFER',
            ...
            'total_amount' => $this->whenLoaded('items', function () {
                return (float) $this->items->reduce(function ($carry, $item) {
                    $price = (string) ($item->product?->unit_price ?? '0');
                    $itemTotal = bcmul($price, (string) $item->quantity, 2);

                    return bcadd((string) $carry, $itemTotal, 2);
                }, '0');
            }),
