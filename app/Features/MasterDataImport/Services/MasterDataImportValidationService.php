...
class MasterDataImportValidationService
{
    ...
    public function validateHeaders(MasterDataImportType $type, array $parsedHeaders): array
    {
        ...
        $allowed = array_merge($canonical, $type->optionalHeaders());
        $errors = [];
        ...
        $unknown = array_diff($parsedHeaders, $allowed);
    ...
    private function validateProducts(array $rows): array
    {
        ...
        foreach ($rows as $item) {
            ...
            // 8. Unit Price decimal validation (optional, 2 decimal places max, no float)
            $unitPriceResult = $this->normalizeUnitPrice(isset($data['unit_price']) ? (string) $data['unit_price'] : null);
            if (! $unitPriceResult['is_valid']) {
                $rowErrors[] = [
                    'row' => $rowNum,
                    'field' => 'unit_price',
                    'code' => 'INVALID_UNIT_PRICE',
                    'message' => $unitPriceResult['error'] ?? 'Harga satuan harus berupa angka desimal non-negatif maksimal 2 digit di belakang koma.',
                ];
                $unitPrice = '0.00';
            } else {
    ...
    /**
     * Normalize and validate unit price value.
     *
     * @return array{is_valid: bool, value: string, error?: string}
     */
    private function normalizeUnitPrice(?string $raw): array
    {
        if ($raw === null) {
            return ['is_valid' => true, 'value' => '0.00'];
        }
        ...
        // Remove prefix 'Rp' or 'Rp.' (case-insensitive) and whitespace
        $clean = preg_replace('/^(rp\.?)\s*/i', '', $clean);
        $clean = trim($clean);

        // Case 1: Indonesian thousand format with dot, optional decimal with comma: e.g. 15.000 or 15.000,50
        if (preg_match('/^\d{1,3}(\.\d{3})+(,\d{1,2})?$/', $clean)) {
            $clean = str_replace('.', '', $clean);
            $clean = str_replace(',', '.', $clean);
        }
        // Case 2: Standard thousand format with comma, optional decimal with dot: e.g. 15,000 or 15,000.50
        elseif (preg_match('/^\d{1,3}(,\d{3})+(\.\d{1,2})?$/', $clean)) {
            $clean = str_replace(',', '', $clean);
        }
        // Case 3: Pure Indonesian decimal with comma: e.g. 15000,50
        elseif (preg_match('/^\d+,\d{1,2}$/', $clean)) {
            $clean = str_replace(',', '.', $clean);
        }
        // Case 4: Standard decimal / integer: e.g. 15000 or 15000.50
        elseif (preg_match('/^\d+(\.\d{1,2})?$/', $clean)) {
            ...
            return [
                'is_valid' => false,
                'value' => '0.00',
                'error' => 'Harga satuan harus berupa angka non-negatif maksimal 2 digit di belakang koma (contoh: 15000 atau 15000.00).',
            ];
        }

        // Check if value exceeds DECIMAL(14,2) max (999999999999.99)
        if (bccomp($clean, '999999999999.99', 2) > 0) {
            return [
                'is_valid' => false,
                'value' => '0.00',
                'error' => 'Harga satuan melebihi batas maksimum (999.999.999.999,99).',
            ];
        }

        return [
            'is_valid' => true,
            'value' => bcadd($clean, '0', 2),
        ];
    }
}
