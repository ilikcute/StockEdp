...
class CommitMasterDataImportAction
{
     ...
     */
    public function execute(MasterDataImportType $type, UploadedFile $file, string $expectedSha256, ?int $userId = null): array
    {
        ...
        // Verify valid user ID or fallback to null to prevent foreign key constraint violations
        $validUserId = ($userId !== null && $userId > 0 && User::where('id', $userId)->exists())
            ? $userId
            : null;

        // 5. Atomic Database Transaction Commit
        DB::transaction(function () use ($type, $normalizedRows, $validUserId): void {
